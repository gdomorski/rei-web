...
export default class Tweets extends Component {
  ...
  async componentDidMount() {
    ...
    if(this.state.prevRoom.length){
      socket.emit('leave room', {room: this.state.prevRoom})
    }
    await socket.emit('stopstream')
    await socket.emit('room', {room: this.props.room, category: this.props.category});
    await this.findTweets(this.props.room)
  }
  ...
  findTweets = async prevRoom => {
    ...
    const response = await fetch(`http://localhost:5000/tweets/${tweetsAboutREI}`)
